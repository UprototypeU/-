<template>
    <div class="billsReceivable" :style="{'min-height':heigh-150+'px'}">
      <div v-if="!backData">
      ...
      <apy-register :backData = "backData" v-if="backData == 'receivableReg'" v-on:headCallBack="headCall"></apy-register>
      <apyCheck :backData = "backData" v-if="backData == 'receivableCheck'" v-on:headCallBack="headCall"></apyCheck>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    hrefBills(is, data) {
      if (is === "支付登记") {
        this.backData = "receivableReg";
      } else {
        this.backData = "receivableCheck";
      }
    }
  },
  watch: {}
