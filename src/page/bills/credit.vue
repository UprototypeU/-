<template>
    <div class="billsCredit" :style="{'min-height':heigh-150+'px'}">
      <div v-if="!backData">
        <p class="title mgt-15 fl">应付账款兑付账单列表</p>
        <div class="search fr">
      ...
      <apy-register :backData = "backData" v-if="backData == 'creditReg'" v-on:headCallBack="headCall"></apy-register>
      <apyCheck :backData = "backData" v-if="backData == 'creditCheck'" v-on:headCallBack="headCall"></apyCheck>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    hrefBills(is, data) {
      if (is === "支付登记") {
        this.backData = "creditReg";
      } else {
        this.backData = "creditCheck";
      }
    }
  },
  watch: {}
